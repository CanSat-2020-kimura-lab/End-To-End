...
def adjustment_angle(azimuth,magx_off,magy_off):
	#--- calculate θ ---#
	...
	try:
		...
		while phaseChk == 7:
			...
			if phaseChk == 8:
				...
				loop_time = time.time()
				stuck_count = 0
				rotate_control_count = 0
				# ------------- GPS navigate ------------- #
				while goal_distance >= 5:
					if goal_distance_before <= goal_distance:
						rotate_control_count += 1
						if rotate_control_count == 3:
							run = pwm_control.Run()
							run.stop()
							time.sleep(1)
							#--- calculate θ ---#
					...
					adjustment_angle(azimuth,magx_off,magy_off)
