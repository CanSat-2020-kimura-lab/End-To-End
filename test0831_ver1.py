...
if __name__ == '__main__':
	try:
		...
		if phaseChk == 2:
			IM920.Send('P2S')
			Other.saveLog(phaseLog, '2', 'Release Phase Started', time.time() - t_start)
		...
		if phaseChk == 3:
			IM920.Send('P3S')
			Other.saveLog(phaseLog, '3', 'Landing Phase Started', time.time() - t_start)
		...
		if phaseChk == 4:
			IM920.Send('P4S')
			Other.saveLog(phaseLog, '4', 'Melting Phase Started', time.time() - t_start)
		...
		if phaseChk == 5:
			IM920.Send('P5S')
			Other.saveLog(phaseLog, '5', 'Mission Phase Strted', time.time() - t_start)
			Other.saveLog(missionLog, 'Mission Start', time.time() - t_start, GPS.readGPS())
			t_mission_start = time.time()
			print('Mission Phase Started {}'.format(time.time() - t_start))
		...
		if phaseChk == 9:
			IM920.Send('P9S')
			Other.saveLog(phaseLog, '9', 'Mission Phase Strted', time.time() - t_start)
			Other.saveLog(missionLog, 'Mission Start', time.time() - t_start, GPS.readGPS())
			t_mission_start = time.time()
			print('Mission Phase Started {}'.format(time.time() - t_start))
			with picamera.PiCamera() as camera:
		...
		if phaseChk == 10:
			IM920.Send('P10S')
			Other.saveLog(phaseLog, '10', 'Goal Detection Phase Started', time.time() - t_start)
			...
			try:
				goalflug = 1
				while goalflug != 0:
					goalflug, goalarea, goalGAP, photoname = goaldetection.GoalDetection("/home/pi/photo/photo",200 ,20, 80, 50000)
